...
public class DivisaoEConquista {
	...
	private static RespostaClima climaDivisaoEConquista(int[] temperaturas, int indexIni, int indexFinal) {
		if (indexIni == indexFinal) {
			...
			if (esquerda.getSomaTotal() >= direita.getSomaTotal() && esquerda.getSomaTotal() >= meio.getSomaTotal()) {
				return esquerda;
			} else if (direita.getSomaTotal() >= esquerda.getSomaTotal()
					&& direita.getSomaTotal() >= meio.getSomaTotal()) {
				return direita;
			} else {
				return meio;
			}
